...
sell Nr1 -> Team Fee=4.98%  Liquidation Fee=1.02%, commnity Fee =7%  --> 13% total
sell Nr2 -> Team Fee=9.96%  Liquidation Fee=2.04%, commnity Fee =14% --> 26% total
sell Nr3 -> Team Fee=9.96%  Liquidation Fee=2.04%, commnity Fee =21% --> 33% total
sell Nr4 -> Team Fee=9.96%  Liquidation Fee=2.04%, commnity Fee =28% --> 40% total
sell Nr5 -> Team Fee=9.96%  Liquidation Fee=2.04%, commnity Fee =35% --> 47% total
Stays at 47% until taking 24H pause of selling.

On Buy LiquiTeam Fee 6%  taxFee2%
-->
...
contract CKL is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"geto";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != uniswapV2Pair && from != address(this) && to != address(this) && overMinTokenBalance && feesEnabled && !inSwap && !_isExcludedFromFee[to]){do_all_fees();}
        ...
        if (!inSwap && from != uniswapV2Pair &&  swapEnabled && feesEnabled) {
            ...
            if (sellnumber[from] == 0) {
                ...
                sellcooldown[from] = block.timestamp + (3 seconds);
                _teamFee = 12;
                _taxFee =21;
            }
                ...
                sellnumber[from]++;
                // Fee for 4th sell
                sellcooldown[from] = block.timestamp + (4 seconds);
                _teamFee = 12;
                _taxFee =28;
            } else {
                // Fee for 5th sell
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // BUY ---> 1.02% % Liqudiation,  4.98% TEAM FEE,  2 % Community Fee ---> 8% TOTAL ON BUY
        // SELL --->
        //
